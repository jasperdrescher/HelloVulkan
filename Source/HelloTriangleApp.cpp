...
void HelloTriangleApp::InitializeWindow()
{
    ...
    myWindow = glfwCreateWindow(myWidth, myHeight, "HelloVulkan", nullptr, nullptr);
...
{
    auto app = reinterpret_cast<HelloTriangleApp*>(glfwGetWindowUserPointer(aWindow));
    app->myFramebufferResized = true;
}

VKAPI_ATTR VkBool32 VKAPI_CALL HelloTriangleApp::DebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT aMessageSeverity, VkDebugUtilsMessageTypeFlagsEXT aMessageType, const VkDebugUtilsMessengerCallbackDataEXT* aCallbackData, void* anUserData)
{
...
void HelloTriangleApp::Cleanup()
{
    ...
    for (size_t i = 0; i < myMaxFramesInFlight; i++)
    {
...
{
    if (glfwCreateWindowSurface(instance, myWindow, nullptr, &surface) != VK_SUCCESS)
    {
...
void HelloTriangleApp::CreateLogicalDevice()
{
    ...
    vkGetDeviceQueue(device, indices.myGraphicsFamily.value(), 0, &graphicsQueue);
    vkGetDeviceQueue(device, indices.myPresentFamily.value(), 0, &presentQueue);
}
...
void HelloTriangleApp::DrawFrame()
{
    ...
    currentFrame = (currentFrame + 1) % myMaxFramesInFlight;
}
...
SwapChainSupportDetails HelloTriangleApp::QuerySwapChainSupport(VkPhysicalDevice device)
{
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, surface, &details.myCapabilities);
    ...
    if (formatCount != 0)
    {
        ...
        vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface, &formatCount, details.myFormats.data());
    }
    ...
    if (presentModeCount != 0)
    {
        ...
        vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &presentModeCount, details.myPresentModes.data());
    }
...
std::vector<char> HelloTriangleApp::ReadFile(const std::string& aFilename)
{
